-- Loom Evidence Graph Schema
-- Version: 1.0.0
-- Source: knowledge-acquisition.md section 4.2
...
CREATE INDEX idx_sources_origin ON sources(origin);
CREATE INDEX idx_sources_tier ON sources(tier);
CREATE INDEX idx_sources_reliability ON sources(reliability);
CREATE INDEX idx_sources_last_verified ON sources(last_verified);
...
CREATE INDEX idx_claims_valid_until ON claims(valid_until);
CREATE INDEX idx_claims_superseded_by ON claims(superseded_by);
...
CREATE INDEX idx_contradictions_resolution ON contradictions(resolution);
